...
class Stock extends CI_Controller
{
    ...
    public function process()
    {
        if (isset($_POST['in_add'])) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $this->session->set_flashdata('pesan', 'Les données de stock ont ​​été ajoutées avec succès');
                redirect('stock/in');
            }
        } else {
            $post = $this->input->post(null, true);
            ...
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('pesan', 'Les données de rupture de stock ont ​​été ajoutées avec succès');
                redirect('stock/out');
            }
        }
    }
    ...
    public function stock_in_del()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('pesan', 'Les données de stock ont ​​été supprimées avec succès');
            redirect('stock/in');
        }
    }
    ...
    public function stock_out_del()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('pesan', 'Les données de rupture de stock ont ​​été supprimées avec succès');
            redirect('stock/out');
        }
    }
}
