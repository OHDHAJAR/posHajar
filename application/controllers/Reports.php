...
class Reports extends CI_Controller
{
    public function __construct()
    {
        ...
        // Load libraries and models after parent constructor
        $this->load->library('fungsi');
        $this->load->model(['sale_m', 'laporan_m']);

        // Custom helper or middleware checks
        check_not_login();
    }
    ...
    public function print_report($sale_id)
    {
        ...
        if (empty($data['report'])) {
            // Handle the error, maybe show a message or redirect
            show_error('Report not found for sale ID: ' . $sale_id);
        }
